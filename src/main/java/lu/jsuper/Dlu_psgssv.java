...
import lu.jsuper.Dlu_slu_mt_util.fact_t;
...
import static lu.jsuper.Dlu_supermatrix.Stype_t.SLU_NR;
import static lu.jsuper.Dlu_supermatrix.Dtype_t.SLU_S;
import static lu.jsuper.Dlu_supermatrix.Mtype_t.SLU_GE;

import static lu.jsuper.Dlu_slu_mt_util.SUPERLU_MAX;
import static lu.jsuper.Dlu_slu_mt_util.fact_t.EQUILIBRATE;
import static lu.jsuper.Dlu_slu_mt_util.yes_no_t.NO;
...
public class Dlu_psgssv {
	 ...
	 */
	    ...
	    SuperMatrix[] AC = new SuperMatrix[1]; /* Matrix postmultiplied by Pc */
		...
		info[0] = -2;
	    else if ( B[0].ncol < 0 || Bstore.lda < SUPERLU_MAX(1, A.nrow) )info[0] = -7;
	    if ( info[0] != 0 ) {
	        i = -(info[0]);
		xerbla_("psgssv", i);
		return;
...
//		AA = (SuperMatrix *) SUPERLU_MALLOC( sizeof(SuperMatrix) );
		AA = sCreate_CompCol_Matrix(A.ncol, A.nrow, Astore.nnz,
	    ...
	    if ( PRNTlevel==1 ) {
	    System.out.printf("nprocs = %d, flops %e, Mflops %.2f\n",
		   nprocs, flopcnt, flopcnt/utime[FACT.ordinal()]*1e-6);
	    System.out.printf("Parameters: w %d, relax %d, maxsuper %d, rowblk %d, colblk %d\n",
