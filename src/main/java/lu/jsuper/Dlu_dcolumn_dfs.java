...
import static lu.jsuper.Dlu_util.CHK_COMPRESS;


public class Dlu_dcolumn_dfs {

	/*! \brief What type of supernodes we want */
	public static boolean T2_SUPER = true;

    private static int first = 1, maxsuper;
		   ...
		   )
	{
		...
		krow = lsub_col[k];
	    	lsub_col[k] = EMPTY;
		kmark = marker2[marker2_offset+krow];
		...
		kperm = perm_r[krow];
	   	...
	   	if ( kperm == EMPTY ) {
		    ...
		    if ( nextl >= nzlmax[0] ) {
		    mem_error = dLUMemXpand(jcol, nextl, MemType.LSUB, nzlmax, Glu);
			if ( mem_error != 0 )
			    return (mem_error);
			lsub = Glu.lsub;
		    }
				...
				xdfs++;
			  	chmark = marker2[marker2_offset+kchild];
			   	...
			   	if ( chmark != jcol ) { /* Not reached yet */
					...
					if ( nextl >= nzlmax[0] ) {
						mem_error =
							dLUMemXpand(jcol,nextl,MemType.LSUB,nzlmax,Glu);
					    if ( mem_error != 0 )
						return (mem_error);
					    lsub = Glu.lsub;
					}
		...
		if ( jsuper == EMPTY ) {	/* starts a new supernode */
		    ...
		    if (CHK_COMPRESS)
			System.out.printf("  Compress lsub[] at super %d-%d\n", fsupc, jcolm1);
