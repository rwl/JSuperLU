...
import static lu.jsuper.Dlu_util.CHK_DFS;
import static lu.jsuper.Dlu_slu_util.EMPTY;


public class Dlu_dpanel_dfs {
			   ...
			   )
	{
		...
		for (k = xa_begin[jj]; k < xa_end[jj]; k++) {
			...
			if (CHK_DFS)
			System.out.printf("krep %d, myfnz %d, perm_r[%d] %d\n", krep, myfnz, krow, kperm);
			...
			if ( myfnz != EMPTY ) {	/* Representative visited before */
			    ...
			    if (CHK_DFS) {
			    System.out.printf("  xdfs %d, maxdfs %d: ", xdfs, maxdfs);
			    for (i = xdfs; i < maxdfs; i++)
			    	System.out.printf(" %d", lsub[i]);
			    System.out.printf("\n");
			    }
			    do {
				/*
				...
				while ( xdfs < maxdfs ) {
					 ...
					 */
					    ...
					    if (CHK_DFS)
					    System.out.printf("chrep %d,myfnz %d,perm_r[%d] %d\n",chrep,myfnz,kchild,chperm);
				...
				if ( marker1[marker1_offset+krep] < jcol ) {
				    segrep[nseg[0]] = krep;
				    ++(nseg[0]);
				    marker1[marker1_offset+krep] = jj;
				}
				...
				if (CHK_DFS) {
				System.out.printf("  pop stack: krep %d,xdfs %d,maxdfs %d: ", krep,xdfs,maxdfs);
				for (i = xdfs; i < maxdfs; i++)
					System.out.printf(" %d", lsub[i]);
				System.out.printf("\n");
			    }
		...
		repfnz_col_offset += m;    /* Move to next column */
		dense_col_offset += m;
