...
import static lu.jsuper.Dlu_slu_mt_util.SUPER_FSUPC;
...
import static lu.jsuper.Dlu.CHK_DFS;
import static lu.jsuper.Dlu.PROFILE;
import static lu.jsuper.Dlu.DEBUG;

import static lu.jsuper.Dlu_util.PrintInt10;


public class Dlu_pdgstrf_panel_dfs {
	 ...
	 */
	    ...
	    int       asub[];
	    int       xa_begin[], xa_end[];
	    int krep, chperm, chmark, chrep, kchild, myfnz;
	    int k, krow, kmark, kperm, fsupc;
	    int xdfs, maxdfs, kpar, jj, nextp;
	    int nextl_col;/* next open position in panel_lsub[*,jj] */
	...
	if (jj == BADCOL)
	    printf("(%d) pdgstrf_panel_dfs[3] %d, krep %d, Pr[kchild %d] %d, chrep %d, fsupc %d, myfnz %d\n",
		   ...
		   SUPER_FSUPC(xsup_end, supno[chrep]), myfnz);
	}
